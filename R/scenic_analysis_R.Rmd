...
params:
  ...
  celltype: "celltype"    # column name for celltype annotations in files
  ...
  zscores_celltype: "./auc_zscores_celltype_annotations.csv"    # Z scores based on AUCell scores for celltypes
  zscores_condition: "./auc_zscores_thirst2_condition_annotations.csv"    # Z scores based on AUCell scores for condition or "None"
  zscores_celltype_condition: "./auc_zscores_celltype_ann_stim_annotations.csv"    # Z scores based on AUCell scores for celltype_condition joint or "None"

  zscore_filter_threshold_celltype: 3    # Z score filter threshold for celltype specific scores for heatmap
  zscore_filter_threshold_condition: 1    # Z score filter threshold for condition specific scores for heatmap or "None"
  zscore_filter_threshold_celltype_condition: 4    # Z score filter threshold for celltype + condition specific scores for heatmap or "None"
  binary_mtx: "./binary_matrix.csv" # binary AUCell scores for each cell
  annotation_celltype: "data.dir/celltype_annotations.csv"    # csv file with cellid & celltype annotations for each cell
  annotation_condition: "data.dir/thirst2_condition_annotations.csv"    # csv file with cellid & condition annotation for each cell or "None"
  annotation_celltype_condition: "data.dir/broad_celltype_stim_annotations.csv"    # csv file with cellid & celltype_condition annotation for each cell or "None"
  ...
  rss_celltype: "./broad_celltype_annotations_RSS.csv"    # RSS scores per regulon for celltype 
  rss_condition: "./thirst2_condition_annotations_RSS.csv"    # RSS scores per regulon for condition or "None"
  rss_celltype_condition: "./broad_celltype_stim_annotations_RSS.csv"    # RSS scores per regulon for celltype_condition or "None"
  species: "human"    # fly, human, mouse or None
  regulon_genes: "./regulons_lucy.csv"    # regulon files generatated from pipeline_pyscenic.py or "None"
  exp_matrix: "data.dir/test.csv"    # filtered-raw-expression.csv exp matrix generated in step 1 of pipeline_pyscenic.py for background for pathway analysis or "None"
...
wilcoxon_pathway_plot <- function(wilcoxon_f = params$wilcoxon_celltype_top, 
  ...
  if (annotation_type != "None") {
     ...
     cc_subset <- merge(comparecluster_df, wilcoxon_top10, 
                        by.x = "Cluster", by.y = "regulon")
     ...
     p1 <- p1 + 
       ...
       theme(legend.title = element_text(size = 7), 
             legend.text = element_text(size = 7))
     p1 <- p1 + 
       facet_wrap(.~ .data[[annotation_type]], scales = "free", shrink = T, 
                  nrow = plot_row,ncol = plot_col) + 
     ...
     ggsave(
       plot = p1,
       filename = paste0(pathwaydb, "_plot_", stat_test, "_top10", 
                         annotation_type, "_", test_type, ".pdf"), 
       path = paste0(plots_directory, "/"), dpi = 300, 
       units = units, height = plot_height, width = plot_width)
...
rss_pathway_plot <- function(rss_f = params$rss_celltype, 
  ...
  if (annotation_type != "None") {
   ...
   cc_subset <- merge(comparecluster_df, rss_scores_long, by.x = "Cluster", by.y = "Regulon")
   gsize <- as.numeric(sub("/\\d+$", "", as.character(cc_subset$GeneRatio)))
   gcsize <- as.numeric(sub("^\\d+/", "", as.character(cc_subset$GeneRatio)))
   cc_subset$GeneRatio <- gsize/gcsize
   cluster <- paste(as.character(cc_subset$Cluster), "(", gcsize, ")", sep = "")
   ...
   p1 <- p1 + 
     ...
     theme(legend.title = element_text(size = 7),
           legend.text = element_text(size = 7))
   p1 <- p1 + 
     facet_wrap(.~ annotation, scales = "free", shrink = T, nrow = plot_row, ncol = plot_col) + 
     theme(strip.background = element_blank(), 
           strip.text = element_text(face = "bold", size = 7))
   p1 <- p1 + 
     enrichplot_theme
   ...
   ggsave(
     plot = p1,
     filename = paste0(pathwaydb, "_plot_RSS_scores_top10", annotation_type, ".pdf"), 
     path = paste0(plots_directory, "/"), dpi = 300, 
     units = units, height = plot_height, width = plot_width)
...
zscore_heatmap(zscores_df = zscores_celltype, 
               plot_height = 9, plot_width = 8, 
               filter_threshold = zscore_filter_threshold_celltype, 
               annotation_type = celltype)
```
...
message("Completed up to line 1133")
...
```{r, fig.height = 4, fig.width = 6}
message("Completed up to line 1149")
# annotation_type is the column names from wilcoxon or KS results file with celltype or condition annotation
if (params$condition != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$wilcoxon_condition_top, 
                        ...
                        res_f = GO_res, plot_row = 0, plot_col = 2)
}
```
...
```{r, fig.height = 4, fig.width = 6}
message("Completed up to line 1163")
# annotation_type is the column names from wilcoxon or KS results file with celltype or condition annotation
if (params$celltype != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$ks_celltype_top, 
                        ...
                        res_f = GO_res, plot_col = 5, plot_row = 4)
}
```
...
if (params$condition != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$ks_condition_top, 
                        ...
                        res_f = GO_res, plot_row = 0, plot_col = 4)
}
```
...
if (params$celltype != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$wilcoxon_celltype_top, 
                        ...
                        res_f = Reactome_res, plot_row = 5, plot_col = 4)
}
```
...
if (params$celltype != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$wilcoxon_celltype_top, 
                        ...
                        res_f = msigdb_c2_res, plot_row = 5, plot_col = 4)
}
```
...
if (params$celltype != "None") {
  wilcoxon_pathway_plot(wilcoxon_f = params$wilcoxon_celltype_top, 
                        ...
                        res_f = KEGG_res, plot_row = 5, plot_col = 4)
}
```
