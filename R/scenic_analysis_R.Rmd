...
output:
  html_document:
    ...
    toc_float:
      ...
      toc_depth: 6
params:
  working_dir: "/Users/lgarner/Documents/Klenerman_group/2020/Analysis/10x_stimulated_MAIT"
  ...
  celltype: "SCT_snn_res.0.3"    # column name for celltype annotations in files
  ...
  zscores_celltype: "../../results/pyscenic/normalised.dir/merged-all.dir/aucell_zscores_cluster-annotation.csv"    # Z scores based on AUCell scores for celltypes
  zscores_condition: "../../results/pyscenic/normalised.dir/merged-all.dir/aucell_zscores_stimulation-annotation.csv"    # Z scores based on AUCell scores for condition or "None"
  ...
  binary_mtx: "../../results/pyscenic/normalised.dir/merged-all.dir/binary_matrix.csv" # binary AUCell scores for each cell
  annotation_celltype: "../../results/merged_all_cluster_annotation.csv"    # csv file with cellid & celltype annotations for each cell
  annotation_condition: "../../results/merged_all_stimulation_annotation.csv"    # csv file with cellid & condition annotation for each cell or "None"
  annotation_celltype_condition: "None"    # csv file with cellid & celltype_condition annotation for each cell or "None"
  binary_heatmap_cell_annotations : "../../results/merged_all_cluster_annotation.csv" # csv file with cellid and annotation data per cell for binary all cells heatmap or "None"
  rss_celltype: "../../results/pyscenic/normalised.dir/merged-all.dir/cluster-annotation_RSS.csv"    # RSS scores per regulon for celltype
  rss_condition: "../../results/pyscenic/normalised.dir/merged-all.dir/stimulation-annotation_RSS.csv"    # RSS scores per regulon for condition or "None"
  rss_celltype_condition: "None"    # RSS scores per regulon for celltype_condition or "None"
  species: "human"    # fly, human, mouse or None
  regulon_genes: "../../results/pyscenic/normalised.dir/merged-all.dir/regulons.csv"    # regulon files generatated from pipeline_pyscenic.py or "None"
  exp_matrix: "../../results/pyscenic/normalised.dir/merged-all.dir/filtered-expression.csv"    # filtered-raw-expression.csv exp matrix generated in step 1 of pipeline_pyscenic.py for background for pathway analysis or "None"
  go_ont_option: "BP"    # "BP", "MF", "CC" or "ALL" for all three. Default is "MF" in ClusterProfiler
  diff_exp_test: "MAST"
  celltype_top: "MAST_celltype_top10.csv"    # or "None"
  condition_reference_top: "MAST_condition_reference_top10.csv"   # or "None", results from comparison to reference condition
  condition_pairwise_top: "MAST_condition_pairwise_top10.csv"
...
binary_celltype_perc_heatmap <- function(cellInfo_f = params$annotation_celltype,
    ...
    if (annotation_type != "None"){
        ...
        save_pheatmap_pdf(
            plot_1,
            filename = paste0(plots_directory, '/', annotation_type,
                              '_regulon_active_perc_cells', minPerc, '_heatmap.pdf'),
            width = plot_width, height = plot_height)
        
        saveRDS(
            plot_1,
            file = paste0(plots_directory, '/', annotation_type,
                          '_regulon_active_perc_cells', minPerc, '_heatmap.rds'))
...
binary_auc_heatmap <- function(binary_mtx_f = params$binary_mtx,
    ...
    annotation_col[[1]] <- as.factor(annotation_col[[1]])
...
pathway_plot <- function(top_regulons_f = params$celltype_top,
                         annotation_type = "celltype",
                         test_type = "one_vs_all",
                         plot_height = 8, plot_width = 8, units = "in",
                         showCategory = 8, stat_test = params$diff_exp_test,
                         pathwaydb = "GO_term", res_f = GO_res,
                         plot_row = 1, plot_col = 1) {
    comparecluster_df <- res_f@compareClusterResult
    
    if (annotation_type == "pairwise") {
        top10 <- read_csv(file = paste0(results_directory, "/", top_regulons_f))
        annotation_type <- "comparison"
        top10 <- top10 %>%
            unite(comparison, test_condition:reference_condition)
    }
    
    if (!annotation_type %in% c("None", "comparison")) {
        top10 <- read_csv(file = paste0(results_directory, "/", top_regulons_f))
    }
    ...
    if (annotation_type != "None") {
        ...
        cc_subset <- comparecluster_df %>%
            inner_join(top10, by = c("Cluster" = "regulon"))
        ...
        if (nrow(cc_subset) > 0) {
            ...
            pdf(paste0(plots_directory, "/", pathwaydb, "_plot_", stat_test,
                       "_top10_", annotation_type, "_", test_type, ".pdf"),
            ...
            print("No overlap between comparecluster_df and top10 Wilcoxon (or chosen test)/KS")
        }
        
    } else {
        print("Wilcoxon (or chosen test)/KS files cell or condition column name in results file not inputted in function")
    }
}
...
reactome_pathway_enrichment <- function(regulons_f = params$regulon_gene,
    ...
    if (params$species == "fly") {
        ...
        bg_id_v <- as.character(c(bg_id$ENTREZID))
        gene_id_list <- foreach(g = genes_list) %do% {
            id_df_g <- bitr(g, fromType = "SYMBOL", toType = "ENTREZID", 
                            OrgDb = "org.Hs.eg.db")
            id_list_g <- as.character(c(id_df_g$ENTREZID))
        }
        names(gene_id_list) <- names(genes_list)
...
if (params$celltype_condition != "None") {
    zscores_celltype_condition <- read.csv(file = params$zscores_celltype_condition, 
                                           header = T)
...
if (params$condition != "None") {
    binary_celltype_perc_heatmap(plot_height = 4, plot_width = 6,
                                 ...
                                 minPerc = 0.40, cluster_rows = F, cluster_cols = F)
} else {
...
### Top 10 Wilcoxon (or chosen test) regulons by celltype (one vs all)

```{r, fig.height = 7, fig.width = 8}
# annotation_type is the column names from Wilcoxon (or chosen test) or KS results file with celltype or condition annotation
if (params$celltype != "None") {
    pathway_plot(top_regulons_f = params$celltype_top,
                 annotation_type = "celltype", test_type = "one_vs_all",
                 plot_height = 10, plot_width = 11, units = "in",
                 showCategory = 7, stat_test = params$diff_exp_test,
                 pathwaydb = "GO_term", res_f = GO_res,
                 plot_row = 1, plot_col = 1)
}
```

### Top 10 Wilcoxon (or chosen test) regulons by condition (compared to reference)

```{r, fig.height = 4, fig.width = 6}
# annotation_type is the column names from Wilcoxon (or chosen test) or KS results file with celltype or condition annotation
if (params$condition != "None") {
    pathway_plot(top_regulons_f = params$condition_reference_top,
                 annotation_type = "condition", test_type = "reference",
...
### Top 10 Wilcoxon (or chosen test) regulons by condition (pairwise)

```{r, fig.height = 4, fig.width = 6}
# annotation_type is the column names from Wilcoxon (or chosen test) or KS results file with celltype or condition annotation
if (params$condition != "None") {
    pathway_plot(top_regulons_f = params$condition_pairwise_top,
                 annotation_type = "pairwise", test_type = "pairwise",
                 plot_height = 6, plot_width = 8, units = "in",
                 showCategory = 7, stat_test = params$diff_exp_test, 
                 pathwaydb = "GO_term", res_f = GO_res, 
                 plot_row = 1, plot_col = 1)
...
# annotation_type is the column names from Wilcoxon (or chosen test) or KS results file with celltype or condition annotation
if (params$celltype != "None") {
    pathway_plot(top_regulons_f = params$ks_celltype_top,
...
# annotation_type is the column names from Wilcoxon (or chosen test) or KS results file with celltype or condition annotation
if (params$condition != "None") {
    pathway_plot(top_regulons_f = params$ks_condition_top,
