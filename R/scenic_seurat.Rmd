...
annotation_col <- metadata %>%
    select(barcode, !!sym(params$celltype)) %>%
    column_to_rownames(var = "barcode")
...
if (params$condition != "None") {
    aucell_matrix_long <- aucell_matrix %>%
        ...
        pivot_longer(-regulon, names_to = "barcode", values_to = "aucell_score") %>%
        left_join(metadata %>%
                      select(barcode, !!sym(params$condition), !!sym(params$celltype)))
} else {
    aucell_matrix_long <- aucell_matrix %>%
        rownames_to_column(var = "regulon") %>%
        pivot_longer(-regulon, names_to = "barcode", values_to = "aucell_score") %>%
...
write_csv(wilcoxon_markers_celltype,
          paste0(results_directory, "/wilcoxon_celltype_comparison.csv"))
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition,
              paste0(results_directory, "/wilcoxon_condition_onevsall_comparison.csv"))
}
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition_reference,
              paste0(results_directory, "/wilcoxon_condition_reference_comparison.csv"))
}
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition_pairwise,
              paste0(results_directory, "/wilcoxon_condition_pairwise_comparison.csv"))
}
```
...
top10_wilcoxon_celltype <- wilcoxon_markers_celltype %>%
    ...
    filter(as.numeric(p_val_adj) < params$FDR_threshold) %>%
...
top10_ks_celltype <- ks_results_celltype %>%
    group_by(test) %>%
    filter(as.numeric(FDR) < params$FDR_threshold) %>%
    arrange(desc(log2FC)) %>%
    slice_head(n = 10) %>%
    ungroup()
...
top10_heatmap <- function(top10_regulons_table,
    ...
    if (nrow(top10_regulons_table) > 0) {
        ...
        cluster_rows <- ifelse(nrow(top10_regulons_table_filtered) > 2, TRUE, FALSE)
...
if (nrow(top10_wilcoxon_celltype) > 0) {
    ...
    cluster_rows <- ifelse(nrow(top10_wilcoxon_celltype_average) > 2, TRUE, FALSE)
...
if (params$condition != "None") {
    ...
    if (nrow(top10_wilcoxon_condition_reference) > 0) {
        ...
        cluster_rows <- ifelse(nrow(top10_wilcoxon_condition_reference_average) > 2, TRUE, FALSE)
    ...
    if (nrow(top10_wilcoxon_condition_pairwise) > 0) {
        ...
        cluster_rows <- ifelse(nrow(top10_wilcoxon_condition_pairwise_average) > 2, TRUE, FALSE)
...
if (nrow(top10_ks_celltype) > 0) {
    ...
    cluster_rows <- ifelse(nrow(top10_ks_celltype_average) > 2, TRUE, FALSE)
...
if (params$condition != "None" & nrow(top10_ks_condition) > 0) {
    ...
    cluster_rows <- ifelse(nrow(top10_ks_condition_average) > 2, TRUE, FALSE)
