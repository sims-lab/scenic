...
plot_violin_aucell <- function(seurat_object, feature, group_by, split_by = "None", name = "aucell_score_violin") {
    ...
    if (split_by == "None") {
        ...
        print(VlnPlot(seurat_object, features = feature, assay = "scenic", slot = "scale.data", pt.size = 0.25) +
                  labs(y = "AUCell score") +
...
for (identity in as.character(unique(Idents(seurat_object)))) {
    markers <- FindMarkers(seurat_object, slot = "scale.data", ident.1 = identity, only.pos = TRUE, verbose = FALSE)
...
write_csv(wilcoxon_markers_celltype, paste0(results_directory, "/", datatype, "/", "wilcoxon_celltype_comparison_FindMarkers.csv"))
...
if (params$condition != "None") {
    ...
    for (identity in test_identities) {
        markers <- FindMarkers(seurat_object, slot = "scale.data", ident.1 = identity, ident.2 = params$reference_condition, only.pos = TRUE, verbose = FALSE)
...
if (params$condition != "None") {
    ...
    for (i in 1:length(identities)) {
        ...
        for (k in 1:length(reference_identities)) {
            markers <- FindMarkers(seurat_object, slot = "scale.data", ident.1 = test_identity, ident.2 = reference_identities[k], only.pos = TRUE, verbose = FALSE)
    ...
    write_csv(wilcoxon_markers_condition_pairwise, paste0(results_directory, "/", datatype, "/", "wilcoxon_condition_pairwise_comparison_FindMarkers.csv"))
}
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition, paste0(results_directory, "/", datatype, "/", "wilcoxon_condition_onevsall_comparison_FindMarkers.csv"))
}
```
...
top10_wilcoxon_celltype <- wilcoxon_markers_celltype %>%
    ...
    slice_head(n = 10) %>%
    ungroup()
write_csv(top10_wilcoxon_celltype, paste0(results_directory, "/", datatype, "/", "wilcoxon_celltype_top10_FindMarkers.csv"))
...
if (params$condition != "None") {
    top10_wilcoxon_condition_reference <- wilcoxon_markers_condition_reference %>%
        ...
        slice_head(n = 10) %>%
        ungroup()
    write_csv(top10_wilcoxon_condition_reference, paste0(results_directory, "/", datatype, "/", "wilcoxon_condition_reference_top10_FindMarkers.csv"))
    ...
    top10_wilcoxon_condition_pairwise <- wilcoxon_markers_condition_pairwise %>% 
        ...
        slice_head(n = 10) %>% 
        ungroup()
    write_csv(top10_wilcoxon_condition_pairwise, paste0(results_directory, "/", datatype, "/", "wilcoxon_condition_pairwise_top10_FindMarkers.csv"))
...
```{r Heatmap_wilcoxon}
# For heatmap, you want to plot AUCell z-scores because you are just interesting in comparing between groups (not between regulons)
top10_heatmap <- function(top10_regulons_table, aucell_zscore, group, filename = "") {
    ...
    print(pheatmap(top10_regulons_table, show_colnames = FALSE,
                   annotation_col = annotation_col, filename = paste0(params$plots_directory, "/", datatype, "/top10_regulons_wilcoxon_", group, filename, "_heatmap.pdf"), width = 8, height = 10))
}

top10_heatmap(top10_wilcoxon_celltype, aucell_zscore, params$cell_type)

if (params$condition != "None") {
    top10_heatmap(top10_wilcoxon_condition_reference, aucell_zscore, params$stimulation, "_reference_")
    top10_heatmap(top10_wilcoxon_condition_pairwise, aucell_zscore, params$stimulation, "_pairwise_")
}
```
...
if (params$condition != "None") {
    ...
    pheatmap(top10_wilcoxon_condition_reference_average, scale = "row",
             filename = paste0(params$plots_directory, "/", datatype, "/top10_regulons_wilcoxon_condition_reference_average_heatmap.pdf"), width = 5, height = 6)
    
    top10_wilcoxon_condition_pairwise_average <- AverageExpression(seurat_object, features = unique(top10_wilcoxon_condition_pairwise$regulon), assays = "scenic", slot = "scale.data")
    top10_wilcoxon_condition_pairwise_average <- top10_wilcoxon_condition_pairwise_average[["scenic"]]
    pheatmap(top10_wilcoxon_condition_pairwise_average, scale = "row",
             filename = paste0(params$plots_directory, "/", datatype, "/top10_regulons_wilcoxon_condition_pairwise_average_heatmap.pdf"), width = 5, height = 6)
}
```
