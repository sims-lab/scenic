...
saveRDS(seurat_object,
        file = paste0(results_directory, "/scenic_seurat_object.rds"))
```
...
if (params$condition != "None") {
    ...
    if (length(unique(top10_regulons_per_condition$regulon)) == length(top10_regulons_per_condition$regulon)) {
        ...
        pheatmap(top10_regulons_per_condition_aucell,
                 show_colnames = FALSE,
                 annotation_col = annotation_col,
                 annotation_row = annotation_row,
                 filename = paste0(plots_directory, "/top10_regulons_per_",
                                   params$condition, "_z-score_heatmap.pdf"),
                 width = 8, height = 6)
    } else {
        pheatmap(top10_regulons_per_condition_aucell,
                 show_colnames = FALSE,
...
if (params$condition != "None") {
    ...
    pheatmap(top10_regulons_per_condition_abs_aucell,
             show_colnames = FALSE,
             annotation_col = annotation_col,
             filename = paste0(plots_directory, "/top10_regulons_per_",
...
markers_celltype <- markers_celltype %>%
    dplyr::rename(celltype = cluster, regulon = gene)
...
if (nrow(top10_celltype) > 0) {
    top10_celltype_average <- as.data.frame(average_aucell_celltype) %>%
...
if (params$condition != "None") {
    ...
    if (nrow(top10_condition) > 0) {
        top10_condition_average <- as.data.frame(average_aucell_condition) %>%
    ...
    if (nrow(top10_condition_reference) > 0) {
        top10_condition_reference_average <- as.data.frame(average_aucell_condition) %>%
    ...
    if (nrow(top10_condition_pairwise) > 0) {
        top10_condition_pairwise_average <- as.data.frame(average_aucell_condition) %>%
...
if (params$condition != "None") {
    if (params$celltype_condition != "None") {
        ...
        if (nrow(top10_cellsubtype_condition_reference) > 0) {
            top10_celltype_condition_reference_average <- as.data.frame(average_aucell_celltype_condition) %>%
        ...
        if (nrow(top10_cellsubtype_condition_pairwise) > 0) {
            top10_celltype_condition_pairwise_average <- as.data.frame(average_aucell_celltype_condition) %>%
