...
params:
    working_dir: "~/Documents/CNCB/proj009/Vincent_proj009/cellranger/Latest_RMD/Results/SCENIC/Glia/normalised/results"
    seurat_object: "./Thirst2_TrimPlus_Glia.rds"
    aucell_scores: "./Thirst2Glia_aucell.csv"
    aucell_zscores: "./aucell_zscores.csv"
    aucell_zscores_celltype: "./aucell_zscores_Glia_celltype_annotations.csv"
    aucell_zscores_condition: "./aucell_zscores_Glia_condition_annotations.csv"
    ...
    stacked_vln_function: "~/Documents/Pyscenic_lucy/scenic/R/stacked_violin.R"
    dotplot_function: "~/Documents/Pyscenic_lucy/scenic/R/seurat_dotplot.R"
    reference_condition: "sat00"
    celltype_condition: TRUE
...
6. Performs Wilcoxon and KS tests to identify regulons that show differential activity between cell types and conditions
...
try({top10_regulons_per_condition_abs <- top10_regulons_per_group_abs(
...
#### Wilcoxon and KS test to identify regulons that are differentially-expressed between conditions or cell types or celltypes_conditions
...
write_csv(wilcoxon_markers_celltype,
          paste0(results_directory, 
                 "/wilcoxon_celltype_comparison.csv"))
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition,
              paste0(results_directory, 
                     "/wilcoxon_condition_onevsall_comparison.csv"))
}
...
if (params$condition != "None") {
    ...
    write_csv(wilcoxon_markers_condition_reference,
              paste0(results_directory, 
                     "/wilcoxon_condition_reference_comparison.csv"))
}
...
if (params$condition != "None") {
    ...
    for (i in 1:length(identities)) {
        ...
        for (k in 1:length(reference_identities)) {
            ...
            markers <- rownames_to_column(markers, var = "regulon")
            wilcoxon_markers_list[[paste(test_identity, reference_identities[k],
                                         sep = "_")]] <- markers
        }
    }
    wilcoxon_markers_condition_pairwise <- bind_rows(wilcoxon_markers_list)
    write_csv(wilcoxon_markers_condition_pairwise,
              paste0(results_directory, 
                     "/wilcoxon_condition_pairwise_comparison.csv"))
}


# Cell type_Condition - test vs. reference (Test conditions, within a cell type)
if (params$condition != "None"){
    if(params$celltype_condition == "TRUE"){
        Idents(seurat_object) <- params$celltype
        DefaultAssay(seurat_object) <- "scenic"
        wilcoxon_markers_list <- list()
        wilcoxon_celltype_list <- list()
        for(iden in as.character(unique(Idents(seurat_object)))) {
            ...
            test_identities <- setdiff(as.character(unique(Idents(cellsubtype))), params$reference_condition)
            for(cond in test_identities){
                ...
                markers <- rownames_to_column(markers, var = "regulon")
                wilcoxon_markers_list[[cond]] <- markers
    }
        wilcoxon_markers_condition_reference <- bind_rows(wilcoxon_markers_list)
        wilcoxon_celltype_list[[iden]] <- wilcoxon_markers_condition_reference
  }
  wilcoxon_markers_cellsubtype_condition_reference <- bind_rows(wilcoxon_celltype_list)
  write_csv(wilcoxon_markers_cellsubtype_condition_reference, file =     paste0(results_directory,"wilcoxon_celltype_condition_reference_comparison.csv")) 
    } else {
...
# Cell type_Condition -  all paiwise (Test conditions, within a cell type)
# Condition - all pairwise
if(params$condition != "None"){
    if(params$celltype_condition == "TRUE"){
     Idents(seurat_object) <- params$celltype
     ...
     for(iden in as.character(unique(Idents(seurat_object)))) {
         ...
         for (i in 1:length(identities)) {
             ...
             wilcoxon_markers_condition_pairwise <- bind_rows(wilcoxon_markers_list)
             wilcoxon_celltype_list[[iden]] <- wilcoxon_markers_condition_pairwise
    }
    
}
  wilcoxon_markers_cellsubtype_condition_pairwise <- bind_rows(wilcoxon_celltype_list)
  write_csv(wilcoxon_markers_cellsubtype_condition_pairwise, 
            file =     paste0(results_directory,"wilcoxon_celltype_condition_pairwise_comparison.csv")) 
...
try({ks_results_condition <- all_regulons_comparison(condition_comparisons,
                                                     aucell_matrix_long,
                                                     params$condition,
                                                     "KS")
...
#### Top 10 regulons per group based on Wilcoxon/KS results
...
write_csv(top10_wilcoxon_celltype,
          paste0(results_directory, "/wilcoxon_celltype_top10.csv"))
...
if (params$condition != "None") {
     top10_wilcoxon_condition <- wilcoxon_markers_condition %>%
    ...
    write_csv(top10_wilcoxon_condition,
              paste0(results_directory, "/wilcoxon_condition_top10.csv"))
    ...
    write_csv(top10_wilcoxon_condition_reference,
              paste0(results_directory, "/wilcoxon_condition_reference_top10.csv"))

    top10_wilcoxon_condition_pairwise <- wilcoxon_markers_condition_pairwise %>%
    ...
    write_csv(top10_wilcoxon_condition_pairwise,
              paste0(results_directory, "/wilcoxon_condition_pairwise_top10.csv"))
    ...
    if(params$celltype_condition == "TRUE"){
        top10_wilcoxon_cellsubtype_condition_reference <- wilcoxon_markers_cellsubtype_condition_reference %>%
        group_by(celltype,test_condition) %>%
        filter(p_val_adj < params$FDR_threshold) %>%
        arrange(desc(avg_diff)) %>%
        slice_head(n=10) %>%
        ungroup()
        write_csv(top10_wilcoxon_cellsubtype_condition_reference,
              paste0(results_directory, "/wilcoxon_celltype_condition_reference_top10.csv"))
        
        top10_wilcoxon_cellsubtype_condition_pairwise <- wilcoxon_markers_cellsubtype_condition_pairwise %>%
        ...
        ungroup()
        write_csv(top10_wilcoxon_cellsubtype_condition_reference,
              paste0(results_directory, "/wilcoxon_celltype_condition_pairwise_top10.csv"))
        
    } else {
        print("celltype_condition parameter set to FALSE")
    }
}
...
top10_heatmap <- function(top10_regulons_table,
    ...
    if (nrow(top10_regulons_table) > 0) {
        ...
        pheatmap(top10_regulons_table_filtered,
                 ...
                 filename = paste0(plots_directory, "/top10_regulons_wilcoxon_",
                                   group, filename, "_heatmap.pdf"),
                 width = 8, height = 10)
...
top10_heatmap(top10_wilcoxon_celltype, aucell_zscore, params$celltype)

if (params$condition != "None") {
    top10_heatmap(top10_wilcoxon_condition_reference,
                  aucell_zscore,
                  params$condition,
                  "_reference")

    top10_heatmap(top10_wilcoxon_condition_pairwise,
                  aucell_zscore,
                  params$condition,
                  "_pairwise")
}
...
if (nrow(top10_wilcoxon_celltype) > 0) {
    top10_wilcoxon_celltype_average <- average_aucell_celltype %>% 
    filter(rownames(.) %in% top10_wilcoxon_celltype$regulon)

    cluster_rows <- ifelse(nrow(top10_wilcoxon_celltype_average) > 2, 
                           TRUE, FALSE)
    pheatmap(top10_wilcoxon_celltype_average,
             scale = "row",
             cluster_rows = cluster_rows,
             filename = paste0(plots_directory, "/top10_regulons_wilcoxon_",
    ...
    "top10_wilcoxon_celltype is empty"
}
...
if (params$condition != "None") {
    ...
    if(nrow(top10_wilcoxon_condition) >0) {
        top10_wilcoxon_condition_average <- average_aucell_condition %>%
            filter(rownames(.) %in% top10_wilcoxon_condition$regulon)
        cluster_rows <- ifelse(nrow(top10_wilcoxon_condition_average) > 2, 
                               TRUE, FALSE)
        pheatmap(top10_wilcoxon_condition_average,
                 scale = "row",
                 cluster_rows = cluster_rows,
                 filename = paste0(plots_directory,
                                   "/top10_regulons_wilcoxon_condition_average_heatmap.pdf"),
                 width = 5, height = 8,
                 angle_col = 90)
        
    } else{
        "top10_wilcoxon_condition is empty"
    }

    if (nrow(top10_wilcoxon_condition_reference) > 0) {
        top10_wilcoxon_condition_reference_average <- average_aucell_condition %>%
        filter(rownames(.) %in% top10_wilcoxon_condition_reference$regulon)

        cluster_rows <- ifelse(nrow(top10_wilcoxon_condition_reference_average) > 2, 
                               TRUE, FALSE)
        pheatmap(top10_wilcoxon_condition_reference_average,
                 scale = "row",
                 cluster_rows = cluster_rows,
                 filename = paste0(plots_directory,
                                   "/top10_regulons_wilcoxon_condition_reference_average_heatmap.pdf"),
                 width = 5, height = 8,
                 angle_col = 90)

    } else {
        "top10_wilcoxon_condition_reference is empty"
    }

    if (nrow(top10_wilcoxon_condition_pairwise) > 0) {
        top10_wilcoxon_condition_pairwise_average <- average_aucell_condition %>%
            filter(rownames(.) %in% top10_wilcoxon_condition_pairwise$regulon)
        
        cluster_rows <- ifelse(nrow(top10_wilcoxon_condition_pairwise_average) > 2, 
                               TRUE, FALSE)
        pheatmap(top10_wilcoxon_condition_pairwise_average,
                 scale = "row",
                 cluster_rows = cluster_rows,
                 filename = paste0(plots_directory,
                                   "/top10_regulons_wilcoxon_condition_pairwise_average_heatmap.pdf"),
                 width = 5, height = 8,
                 angle_col = 90)
    } else {
        "top10_wilcoxon_condition_pairwise is empty"
    }
...
if(params$condition != "None") {
    if(params$celltype_condition == "TRUE"){
         metadata <- metadata %>% unite(col = "celltype_condition", c(!!sym(params$celltype), !!sym(params$condition)), sep = "_" )
        ...
        if (nrow(top10_wilcoxon_cellsubtype_condition_reference) > 0) {
            top10_wilcoxon_celltype_condition_reference_average <- average_aucell_celltype_condition %>%
            filter(rownames(.) %in% top10_wilcoxon_cellsubtype_condition_reference$regulon)

            cluster_rows <- ifelse(nrow(top10_wilcoxon_celltype_condition_reference_average) > 2, 
                               TRUE, FALSE)
            pheatmap(top10_wilcoxon_celltype_condition_reference_average,
                 scale = "row",
                 cluster_rows = cluster_rows,fontsize_col = 8,treeheight_col = 10,
                 filename = paste0(plots_directory,
                                   "/top10_regulons_wilcoxon_celltype_condition_reference_average_heatmap.pdf"),
                 width = 8, height = 8,
                 angle_col = 90)

    } else {
        "top10_wilcoxon_cellsubtype_condition_reference is empty"
    }

    if (nrow(top10_wilcoxon_cellsubtype_condition_pairwise) > 0) {
        top10_wilcoxon_celltype_condition_pairwise_average <- average_aucell_celltype_condition %>%
            filter(rownames(.) %in% top10_wilcoxon_cellsubtype_condition_pairwise$regulon)
        
        cluster_rows <- ifelse(nrow(top10_wilcoxon_celltype_condition_pairwise_average) > 2, 
                               TRUE, FALSE)
        pheatmap(top10_wilcoxon_celltype_condition_pairwise_average,
                 scale = "row",
                 cluster_rows = cluster_rows,fontsize_col = 8,treeheight_col = 10,
                 filename = paste0(plots_directory,
                                   "/top10_regulons_wilcoxon_celltype_condition_pairwise_average_heatmap.pdf"),
                 width = 8, height = 8,
                 angle_col = 90)
    } else {
        "top10_wilcoxon_cellsubtype_condition_pairwise is empty"
    }
...
if (nrow(top10_ks_celltype) > 0) {
    ...
    if( params$condition != "None" & (nrow(top10_ks_condition)) > 0) {
    Idents(seurat_object) <- params$condition
...
if (nrow(top10_wilcoxon_celltype) > 0) {
    top10_wilcoxon_celltype <- top10_wilcoxon_celltype %>%
    group_by(test_cell_type) %>%
    filter(p_val_adj < params$FDR_threshold) %>%
    arrange(desc(avg_diff)) %>%
    slice_head(n = 5) %>%
    ungroup()
    Idents(seurat_object) <- params$celltype
    cluster.idents <- ifelse(nrow(top10_wilcoxon_celltype) > 2, 
                               TRUE, FALSE)
    p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_celltype$regulon),
            cols = "RdYlBu", col.max = 3, col.min = -3,cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
    p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
    ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_",
                               params$celltype, "_dotplot.pdf"),width = 8,height=6)
  ...
  if (params$condition != "None"){
      ...
      if(nrow(top10_wilcoxon_condition) > 0){
        Idents(seurat_object) <- params$condition
        cluster.idents <- ifelse(nrow(top10_wilcoxon_condition) > 2, 
                               TRUE, FALSE)
         p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_condition$regulon),
            cols = "RdYlBu", col.max = 3, col.min = -3,cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
         p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
         ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_condition_dotplot.pdf"),width = 8,height=6)
       ...
       if(nrow(top10_wilcoxon_condition_reference) > 0){
        Idents(seurat_object) <- params$condition
        cluster.idents <- ifelse(nrow(top10_wilcoxon_condition_reference) > 2, 
                               TRUE, FALSE)
         p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_condition_reference$regulon),
            cols = "RdYlBu", col.max = 3, col.min = -3,cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
         p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
         ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_condition_reference_dotplot.pdf"),width = 8,height=6)
    ...
    if (nrow(top10_wilcoxon_condition_pairwise) > 0) {
        Idents(seurat_object) <- params$condition
        cluster.idents <- ifelse(nrow(top10_wilcoxon_condition_pairwise) > 2, 
                               TRUE, FALSE)
         p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_condition_pairwise$regulon),
            cols = "RdYlBu", col.max = 3, col.min = -3,cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
         p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
         ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_condition_pairwise_dotplot.pdf"),width = 8,height=6)
...
if (params$condition != "None" & params$celltype_condition == "TRUE"){
    
    if(nrow(top10_wilcoxon_cellsubtype_condition_reference) > 0){
             Idents(seurat_object) <- "celltype_condition"
             cluster.idents <- ifelse(nrow(top10_wilcoxon_cellsubtype_condition_reference) > 2, 
                               TRUE, FALSE)
             p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_cellsubtype_condition_reference$regulon),
             cols = "RdYlBu", col.max = 3, col.min = -3,cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
             p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
             ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_celltype_condition_reference_dotplot.pdf"),width = 8,height=6)
     ...
     if (nrow(top10_wilcoxon_cellsubtype_condition_pairwise) > 0) {
        Idents(seurat_object) <- "celltype_condition"
        cluster.idents <- ifelse(nrow(top10_wilcoxon_cellsubtype_condition_pairwise) > 2, 
                               TRUE, FALSE)
         p <- DotPlot(object = seurat_object,assay = "scenic", features = unique(top10_wilcoxon_cellsubtype_condition_pairwise$regulon),
            cols = "RdYlBu", col.max = 3, col.min = -3, cluster.idents = cluster.idents) + RotatedAxis() + FontSize(x.text = 9,y.text = 10,x.title = 8,y.title = 8) 
         p <- p + theme(legend.text = element_text(size=12), legend.title = element_text(size=12))
         ggsave(filename = paste0(plots_directory, "/top10_regulons_wilcoxon_celltype_condition_pairwise_dotplot.pdf"),width = 8,height=6)
