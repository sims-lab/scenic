...
annotation_files = annotation_files.split(',')
...
for annotation in annotation_dict.keys():
    rss_louvain = regulon_specificity_scores(auc_mtx, annotation_dict[annotation])
    rss_louvain.insert(0, annotation, rss_louvain.index.values)
    rss_louvain.to_csv('pyscenic_results.dir/' + annotation + '_RSS.csv', index = False)
...
auc_mtx_Z.to_csv('pyscenic_results.dir/aucell_zscores.csv')
...
for annotation in annotation_dict.keys():
    annotation_info = annotation_dict[annotation]
    aucell_score = auc_mtx_long.merge(annotation_info, left_on = 'cellid', right_index = True).drop('cellid', axis = 1)
    ...
    aucell_score.to_csv('pyscenic_results.dir/aucell_zscores_' + annotation + '.csv', index = False)
    ...
    aucell_score_3.to_csv('pyscenic_results.dir/aucell_zscores_' + annotation + '_specific_regulators_3.csv', index = False)
    ...
    aucell_score_2.to_csv('pyscenic_results.dir/aucell_zscores_' + annotation + '_specific_regulators_2.csv', index = False)
