...
def scenic_seurat(infile, outfile):
    ...
    statement = '''Rscript -e "rmarkdown::render('%(R_PATH)s/scenic_seurat.Rmd',
                        params = list(  working_dir = '%(working_dir)s',
                                        seurat_object = '%(seurat_object)s',
                                        aucell_scores = '%(infile)s',
                                        aucell_zscores= %(aucell_zscores)s,
                                        aucell_zscores_celltype = '%(aucell_zscores_celltype)s'
                                        aucell_zscores_condition = ''%(aucell_zscores_condition)s',
                                        binary_matrix = '%(binary_matrix)s',
                                        ...
                                        stacked_vln_function = '%(stacked_vln_function)s',
    ...
    def rscenic(infile, outfile):
        ...
        statement = '''Rscript -e "rmarkdown::render('%(R_PATH)s/scenic_seurat.Rmd',
                            params = list(  working_dir = '%(working_dir)s',
                                            ...
                                            celltype_zscores = '%(celltype_zscores)s',
                                            celltype_stim_zscores = '%(celltype_stim_zscores)s',
                                            stim_zscores = '%(stim_zscores)s',
                                            ...
                                            binary_mtx = '%(binary_mtx)s',
                                            annotation_celltype = '%(annotation_celltype)s',
                                            annotation_stim = '%(annotation_stim)s',
                                            annotation_celltype_stim = '%(annotation_celltype_stim)s',
                                            binary_heatmap_cell_annotations ='%(binary_heatmap_cell_annotations)s',
                                            rss_celltype = '%(rss_celltype)s',
                                            rss_stim = '%(rss_stim)s',
                                            rss_celltype_stim = '%(rss_celltype_stim)s'
                                            species = '%(species)s',
                                            regulon_genes = '%(regulon_genes)s',
